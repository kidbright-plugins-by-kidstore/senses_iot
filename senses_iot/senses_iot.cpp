...
void CONNACKProcess(uint8_t c, senses_iot *mqtt) {
...
senses_iot::senses_iot() {}

void senses_iot::init(void) {
...
int senses_iot::prop_count(void)
{
	return 0;
}

bool senses_iot::prop_name(int index, char *name)
{
	// not supported
	return false;
}

bool senses_iot::prop_unit(int index, char *unit)
{
	// not supported
	return false;
}

bool senses_iot::prop_attr(int index, char *attr)
{
	// not supported
	return false;
}

bool senses_iot::prop_read(int index, char *value)
{
	// not supported
	return false;
}

bool senses_iot::prop_write(int index, char *value)
{
	// not supported
	return false;
}

void senses_iot::process(Driver *drv) {
	wifiConnected = (uint32_t)((GPIO_REG_READ(GPIO_OUT_REG) >> 2) & 1U) == 0;
	
	gpio_set_level(GPIO_NUM_12, isConnected() ? 0 : 1);
}

void senses_iot::config(char* host, uint16_t port, char *clientId, char *username, char *password) {
	this->host = host;
	this->port = port;
...
bool senses_iot::isConnected() {
	return tcpConnected && mqttConnected;
}

void senses_iot::onConnected(MQTTEventCallback cb) {
	onConnected_cb = cb;
}

void senses_iot::publish(char *topic, char *value, uint8_t QoS) {
...
void senses_iot::publish(char *topic, double value, uint8_t QoS) {
	char buff[20];
	sprintf(buff, "%.8g", value);
	this->publish(topic, buff, QoS);
}

void senses_iot::publish(char *topic, int value, uint8_t QoS) {
	char buff[20];
	sprintf(buff, "%d", value);
	this->publish(topic, buff, QoS);
}

void senses_iot::publish(char *topic, bool value, uint8_t QoS) {
	char buff[2] = {value ? '1' : '0', 0};
	this->publish(topic, buff, QoS);
}

void senses_iot::subscribe(char *topic, MQTTEventCallback cb, int maxQoS) {
	if (!isConnected()) return;
...
char* senses_iot::getTopic() {
	return mqttTopic;
}

double senses_iot::getNumber() {
	return atof((char*)mqttPayload);
}

char* senses_iot::getText() {
	return (char*)mqttPayload;
}

// Senses IoT
void senses_iot::connect(char *uid, char *key) {
	this->uid = uid;
	this->device_key = key;
	...
	char client_id[30];
	snprintf(client_id, sizeof(client_id), "wf-%02X:%02X:%02X:%02X:%02X:%02X", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
	this->config("www.sensesiot.net", 3057, client_id, this->uid, (char *)(""));
	this->connect();
}

void senses_iot::send(int slot, char *value) {
	char topic[128];
	snprintf(topic, 128, "data/%s/%d", this->device_key, slot);
	this->publish(topic, value);
}

void senses_iot::send(int slot, double value) {
	char value_str[20];
	snprintf(value_str, 20, "%f", value);
	this->send(slot, value_str);
}

void senses_iot::send(int slot, int value) {
	char value_str[20];
	snprintf(value_str, 20, "%d", value);
	this->send(slot, value_str);
}

void senses_iot::send(int slot, bool value) {
	this->send(slot, value ? 1 : 0);
}

void senses_iot::add_slot_handle(int slot, MQTTEventCallback callback) {
	char *topic = (char *) malloc(100);
	snprintf(topic, 100, "control/%s/%d", this->device_key, slot);
	this->subscribe(topic, callback);
}

bool senses_iot::slot_value_is_on() {
	return strcmp(this->getText(), "on") == 0;
}

double senses_iot::slot_value() {
